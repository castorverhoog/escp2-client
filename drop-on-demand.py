...
#    timeout=0.1 #arduino connectie maken
    ...
    stepsperRotation = 200*16 #one rotation is 3200 steps using the NEMA 17 stepper motor
...
#    timeout=0.1 #arduino connectie maken
    ...
    # stepsperRotation = 64*64 #one rotation is 64*64=4096 steps using the 28BYJ-48 stepper motor
    stepsperRotation = 200*16 #one rotation is 3200 steps using the NEMA 17 stepper motor
    steps = round(moveMicrons.get()/translationRate*stepsperRotation) # abel addition
#    steps = 5/translationRate*stepsperRotation
    distanceStr = str(steps)
    print ("steps to travel")
    print (steps)
...
Label(root, text=u"\u0394"+"x = ").place(x=1100,y=widget_heigt+460)
Label(root, text=u"\u0394"+"y = ").place(x=1100,y=widget_heigt+480)

output_dx = Label(root, textvariable=dx_microns_round)
output_dy = Label(root, textvariable=dy_microns_round)
output_dx.place(x=1130,y=widget_heigt+460)
output_dy.place(x=1130,y=widget_heigt+480)
Label(root, text="microns").place(x=1200,y=widget_heigt+460)
Label(root, text="microns").place(x=1200,y=widget_heigt+480)

output_comment = Label(root, text="Fill in in ESCP2-client (microns):")
output_comment.config(font=headerfont)
output_comment.place(x=1100,y=widget_heigt+530)

output = Entry(root, textvariable=inputESCP2client)
output.place(x=1100,y=widget_heigt+580)
